...
if ($TactFactory['c_wep'] && $TargetPutWep[0] != '711' && $TargetPutWep[0] != '712' && $TargetPutWep[0] != '715' && $TargetPutWep[0] != '718' && $TargetPutWep[0] != '969'){echo "请先回收之前的武器。";postFooter();exit;}
if (!$Game[$actionc] || !$TargetPutWep[0]){echo "没有此装备存在。";postFooter();exit;}
...
if ($TargetPutWep[2]){echo "已进行过专用化改造的装备无法再次进行改造。";postFooter();exit;}
if ($TargetPutWep[1] < $Max_Wep_Exp && $TargetPutWep[0] != 711 && $TargetPutWep[0] != 712 && $TargetPutWep[0] != 715 && $TargetPutWep[0] != 718 && $TargetPutWep[0] != 969){echo "武器经验不足！";postFooter();exit;}
...
if ($ttlused_pt > $TactFactory['c_point'] || $ttlused_pt <= 0){echo "改造点数不足或出错！";postFooter();exit;}

GetWeaponDetails("$TactFactory[c_wep]",'CustWepS');

if (isset($secureCustom)){
        $secureCustom = 1;
        if ($ttlused_pt+($CustWepS['grade']*10) > $TactFactory['c_point'] || $ttlused_pt <= 0){echo "改造点数不足或出错！";postFooter();exit;}
}
else $secureCustom = 0;
...
$Message = "成功\改造了！<br>效果值: $pc_res_suc < 成功\率: $pc_cust_suc";
        ...
        $Message = "改造失败了。<br>效果值: $pc_res_suc > 成功\率: $pc_cust_suc";
        if ($secureCustom){
...
echo "</form>";
echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
echo "<p align=center style=\"font-size: 16pt\">$Message<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"><input type=submit value=\"继续\" onClick=\"freect.submit()\"></p>";
echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";        
...
echo "<tr><td width=400 colspan=2>当武器达25000经验时，可以进行专用化。<br>专用化能够让你改造武器、提升威力和效率。";
echo "<br>条件如下:<br>";
echo " - 武器经验达25000<br>";
echo " - 武器曾没有进行专用化<br>";
echo " - 专用化完成后，武器经验归零。<br>";
echo "请注意，任何置放在熔解炉(包括武器专用化改造的熔解炉)的武器，将会失去所有经验！<br>还有，过份改造可能会令武器及原料永久破坏！请小心！<br>";
echo "另外，原料无法专用化，被置放的原料会直接转化为改造点数。<br>您的改造点数: $TactFactory[c_point]";
echo "<hr></td></tr>";
...
echo "<tr><td colspan=6 align=center>专用化改造的原料库</td></tr>";
echo "<tr><td width=150>即将进行专用化工序的武器:</td>";
if (!$TactFactory['c_wep'])
echo "<td colspan=5>没有";
else {
GetWeaponDetails("$TactFactory[c_wep]",'CustWepS');
...
echo "</td><td><b>改造成功\率: </b><span id=successpc>100</span>%";
echo "</td></tr><tr><td>";
...
if(type == 'pre'){
        mainform.fixedname.value='$Game[gamename]专用';
        namepre.innerText='$Game[gamename]专用';
        namesuf.innerText='';
}
else if(type == 'suf'){
...
function confirmCustom(){
        if (pt_left.innerText > $TactFactory[c_point]){alert('改造点数不足！\\n无法进行改造。');return false;}
                ...
                if (confirm('即将进行改造，请确保所有资料正确。\\n可以开始改造吗？')==true){mainform.actionb.value='start';return true;}
                else {return false;}
        }
}
</script>";
...
echo "<b style=\"color: yellow\">攻击力强化: </b><br>$CustWepS[atk] => <b style=\"color: blue\" id=atkc>$CustWepS[atk]</b> (上限: $AtkMax)<br>使用点数: <select name=\"atkc_pt\" onchange=\"custom('atk');\">";
...
echo "<b style=\"color: yellow\">命中力强化: </b><br>$CustWepS[hit] => <b style=\"color: blue\" id=hitc>$CustWepS[hit]</b> (上限: $HitMax)<br>使用点数: <select name=\"hitc_pt\" onchange=\"custom('hit');\">";
...
echo "<b style=\"color: yellow\">回数增加: </b><br>$CustWepS[rd] => <b style=\"color: blue\" id=rdc>$CustWepS[rd]</b> (上限: $RdMax)<br>使用点数: <select name=\"rdc_pt\" onchange=\"custom('rd');\">";
for($PtUse_Rd=0;$PtUse_Rd <= $TactFactory['c_point'] && $PtUse_Rd <= 100;$PtUse_Rd++){
echo "<option value=$PtUse_Rd>$PtUse_Rd";}
echo "</select>点";
echo "</td><td>";
echo "<b style=\"color: yellow\">能源消耗: </b><br>$CustWepS[enc] => <b style=\"color: blue\" id=encc>$CustWepS[enc]</b> (下限: <span id=enccmin>$ENCMin</span>)<br>使用点数: <select name=\"encc_pt\" onchange=\"custom('enc');\">";
for($PtUse_EN=0;$PtUse_EN <= $TactFactory['c_point'] && $PtUse_EN <= 60;$PtUse_EN++){
echo "<option value=$PtUse_EN>$PtUse_EN";}
echo "</select>点";
echo "</td></tr><tr><td>";
echo "武器名称更变: <input type=text value=\"$Game[gamename]专用\" name=fixedname maxlength=32 onChange=\"ModName(this.value);\"><br>更变类型: <input type=radio name=namefix value=1 checked onclick=\"switchnfix('pre');\"> 字首附加 <input type=radio name=namefix value=2 onclick=\"switchnfix('suf');\"> 字尾附加";
echo "</td><td>名称预览:<br><span id=namepre>$Game[gamename]专用</span>$CustWepS[name]<span id=namesuf></span><sub>?</sub>";
echo "</td></tr>";
echo "<tr>";
echo "<td>保险机制: <input type=checkbox name=secureCustom onClick=custom() value=true> (消耗 ".($CustWepS['grade']*10)." 点改造点数)</td><td>保险机制能消耗一定的改造点数，失败时挽回损坏的武器。</td>";
echo "</tr><tr><td colspan=2 align=center>";
echo "<input type=submit value='确认改造' onClick='return confirmCustom()'>";
echo "</td></tr></table>";
echo "</td></tr>";
}
