...
include('cfu.php');
        ...
        echo "<title>Endless Battle ~ php-eb - ? 2005-2006 v2Alliance</title>";
        echo "</head>";
                
if(!$mode){
        if (ereg('MSIE',$HTTP_USER_AGENT)) $mode = 'default';
        else $mode = "loginA";
}
if($mode == 'loginA'){
...
echo "<tr>";
echo "<td colspan=2 align=center style=\"font-size: 12pt;font-weight: Bold;\">登入聊天室系统</td>";
echo "</tr><tr>";
echo "<td align=right>使用者名称:</td>";
...
exit;
        ...
        $sql_ugnrli = ("SELECT username, password FROM `".$GLOBALS['DBPrefix']."phpeb_user_general_info` WHERE username='". $Pl_Value['USERNAME'] ."'");
                ...
                echo "<center><br><br>使用者名称或密码错误。<br>$Pl_Value[PASSWORD],$Pl_Value[USERNAME]<br><a href=\"index2.php\" target='_top' style=\"text-decoration: none\">回到首页</a>";
                postFooter();
                exit;}
...
//Draw Table for Entering Text
        ...
        echo "if (!chatenter.message.value){alert(\"请输入你的说话。\");return false;}";
        echo "else if (chatenter.tempvar.value == 1 && !chatenter.players.value && (!chatenter.entname.value || chatenter.entname.value == '<<手动输入>>')){alert(\"请指定说话目标人物。\");return false;}";
        echo "else if (chatenter.tempvar.value == 2 && !chatenter.org.value){alert(\"请指定说话目标组织。\");return false;}";
        echo "else {chatenter.action='chat.php?action=speak';chatenter.SbmtBtn.value='请稍等 ".$SpeakIntv." 秒...';chatenter.SbmtBtn.disabled=true;chatenter.submit();chatenter.message.value='';setTimeout(\"chatenter.SbmtBtn.disabled=false;chatenter.SbmtBtn.value='确 认 送 出';\",".round($SpeakIntv*1000).");}}</script>";
        ...
        echo "</select><input type=text name=\"entname\" value=\"<<手动输入>>\" style=\"visibility:hidden;position:absolute;width: 100%;font-size: 9pt; color: #ffffff; background-color: #000000;text-align: center\" onfocus=\"this.value='';this.style.textAlign='left'\" onmouseover=\"this.style.color='yellow'\" onmouseout=\"this.style.color='FFFFFF'\">";
        ...
        echo "<td colspan=4 style=\"border-bottom-style:none;\">";
                echo " 请输入你的说话: <br><center><textarea class=sc name=\"message\" rows=5 wrap=\"hard\" cols=36 style=\"font-size: 9pt; color: #ffffff; border: 1px solid;border-color: #7F7F7F; background-color: #000000;\">";
                echo "</textarea></center>";
        echo "说话对象:</td>";
        echo "</tr>";
        echo "<tr style=\"padding-left: 8px\"><td style=\"border-bottom-style:none;border-top-style:none;border-right-style:none;\"><input checked type=radio value='0' name=type onClick=\"hideSelect('');tempvar.value='0';\">全频</td>";
        ...
        echo "<td style=\"border-bottom-style:none;border-top-style:none;border-left-style:none;border-right-style:none;\"><input type=radio value='2' name=type onClick=\"hideSelect('org');tempvar.value='2';\">组织</td>";
        echo "<td style=\"border-bottom-style:none;border-top-style:none;border-left-style:none;\"><input type=radio value='3' name=type onClick=\"hideSelect('');tempvar.value='3';\" disabled>拍卖</td></tr>";
        
        echo "<tr><td colspan=4 style=\"border-top-style:none;\" align=center><input style=\"font-size: 9pt; color: #ffffff; background-color: #000000;\" type=button value='确 认 送 出' onClick='vldSubmit();' onmouseover=\"this.style.color='yellow'\" onmouseout=\"this.style.color='FFFFFF'\" name=SbmtBtn>";
        echo "<input style=\"font-size: 9pt; color: #ffffff; background-color: #000000;\" type=button value='刷 新 资 讯' onClick=\"chatenter.action='chat.php?action=display';chatenter.submit()\" onmouseover=\"this.style.color='yellow'\" onmouseout=\"this.style.color='FFFFFF'\"></td></tr>";
...
if($mode == 'speak'){
unset($M,$T);
if (!$message){$ErrMsg = "请输入你的说话。<br>";}
if ($type == 1 && !$players && (!$entname || $entname == '<<手动输入>>'))$ErrMsg .= "请指定说话目标人物。<br>";
elseif ($type == 2 && !$org && $org != 0)$ErrMsg .= "请指定说话目标组织。<br>";}
...
if (!$ErrMsg && $mode == 'speak'){
        ...
        $M = eregi_replace("((f.ck)|(fu.k)|(fuc.)|(shit)|(sh\|t)|(sh1t)|(shlt)|(fxxk)|(sucker)|(bitch)|(asshole)|(fucker)|(motherfucker))+",' ',$M);
        $entname = str_replace("[\|\`(--)]+",'',$entname);
        $players = str_replace("[\|\`(--)]+",'',$players);
        if($type == 1)
                {if ($entname && $entname != '<<手动输入>>') {
                        ...
                        $SelName = mysql_fetch_row($SelNameQuery) or die("没有此人物名称");
                        $T = $SelName[0];}
                else $T = "$players";
                }
        elseif($type == 2) $T = "$org";
...
//Start Viewing Section
        ...
        if ($ChatData_All['c_user'] == $Pl_Value['USERNAME'])
                $MsgShow["All"][$MSC] = cfu_time_convert($ChatData_All['c_time'])." 你对全部人说:<font color=coral><B>".$MsgShow["gen"]."</b>";
        else        $MsgShow["All"][$MSC] = cfu_time_convert($ChatData_All['c_time'])." <font color=$A[1]>$A[0]</font> 对全部人说:<font color=$A[1]>".$MsgShow["gen"];
        ...
        else         $MsgShow["Priv"][$MSC] = cfu_time_convert($ChatData_Priv['c_time'])." <font color=$A[1]>$A[0]</font> 对你俏俏话:<font color=$A[1]>".$MsgShow["gen"]; 
        ...
        else          $MsgShow["Orgz"][$MSC] = cfu_time_convert($ChatData_Orgz['c_time'])." <font color=$A[3]>$A[2]</font> 对 <font color=$A[1]>$A[0]</font> 说:<font color=$A[3]>".$MsgShow["gen"];
