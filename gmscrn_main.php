...
if ( $mode == 'proc' )
        {        $sql_ugnrli = ("SELECT * FROM `".$GLOBALS['DBPrefix']."phpeb_user_general_info` WHERE username='". $Pl_Value['USERNAME'] ."'");
                $UsrGenrl_Qr = mysql_query ($sql_ugnrli) or die ('出错, 原因:' . mysql_error() . '，');
                ...
                $PlGame_Qr = mysql_query ($sql_ugmi) or die ('出错, 原因:' . mysql_error() . '，');
                $PlGameVal = mysql_fetch_array($PlGame_Qr);
                if ($CFU_Time >= $TIMEAUTH+$TIME_OUT_TIME || $TIMEAUTH <= $CFU_Time-$TIME_OUT_TIME){echo "<center>连线逾时！<br>请重新登入！<br><a href=\"index2.php\" target='_top' style=\"text-decoration: none\">回到首页</a>";exit;}
...
if ($UsrGenrl['fame'] >= 0)$TypeFame = '名声';
else $TypeFame = '恶名';
...
echo "    <font style=\"font-size: 10pt\">$TypeFame: $ShowFame    所在区域: $UsrGenrl[coordinates]    名称: $AreaLandForm    统治国家: <font style=\"color: $AreaOrg[color]\">".$AreaOrg[name]."</font>";
if ($Otp_TellTime)
echo " <font style=\"color: red\">[战争状态]</font> $Otp_TellTime";
...
echo "<br>$PlMs[msname]<br> 胜利积分/次数: $PlGameVal[v_points]/$PlGameVal[victory]<br>状态: <b id=status_now style=\"color: $StatusColor\">$StatusShow</b></td>"
,"<td width=\"2%\">　</td>"
...
echo "        if (type == 'at'){";
echo "        if ($UsrGenrl[growth] < $At_Stat_Req || $At_Stat_Req == '0' || $UsrGenrl[growth] == '0'){alert('你的成长点数不足够！');return false}";
echo "        if (confirm('你现在有 $UsrGenrl[growth] 成长点数。\\n要把攻击技术加到 $NextStatPt_At 的话需要 $At_Stat_Req 点数。\\n确定吗?') == true){";
...
echo "        }";
echo "        if (type == 'de'){";
echo "        if ($UsrGenrl[growth] < $De_Stat_Req || $De_Stat_Req == '0' || $UsrGenrl[growth] == '0'){alert('你的成长点数不足够！');return false}";
echo "        if (confirm('你现在有 $UsrGenrl[growth] 成长点数。\\n要把防御能力加到 $NextStatPt_De 的话需要 $De_Stat_Req 点数。\\n确定吗?') == true){";
...
echo "        }";
echo "        if (type == 're'){";
echo "        if ($UsrGenrl[growth] < $Re_Stat_Req || $Re_Stat_Req == '0' || $UsrGenrl[growth] == '0'){alert('你的成长点数不足够！');return false}";
echo "        if (confirm('你现在有 $UsrGenrl[growth] 成长点数。\\n要把反应加到 $NextStatPt_Re 的话需要 $Re_Stat_Req 点数。\\n确定吗?') == true){";
...
echo "        }";
echo "        if (type == 'ta'){";
echo "        if ($UsrGenrl[growth] < $Ta_Stat_Req || $Ta_Stat_Req == '0' || $UsrGenrl[growth] == '0'){alert('你的成长点数不足够！');return false}";
echo "        if (confirm('你现在有 $UsrGenrl[growth] 成长点数。\\n要把命中能力加到 $NextStatPt_Ta 的话需要 $Ta_Stat_Req 点数。\\n确定吗?') == true){";
...
echo "<td rowspan=3 align=center><span id=attacking$AtAdd>攻击</span><br>";
echo "<b style=\"color:$AtClr;\">$PlGameVal[attacking] + $Pl_ATF</b></td>";
echo "<td align=center><span id=defending$DeAdd>防御</span><br>";
echo "<b style=\"color:$DeClr;\">$PlGameVal[defending] + $Pl_DEF</b></td>";
echo "<td rowspan=3 align=center><span id=reacting$ReAdd>反应</span><br>";
...
if ($UsrGenrl['msuit'])echo "<b>生命值</b></td>";
else echo "<b>生命付加值</b></td>";                                                        
...
echo "<td style=\"background: ".$BStyleC.";font-size:13px;\" align=center width=\"70\">";
if ($UsrGenrl['msuit'])echo "<b>能量值</b></td>";
else echo "<b>能量付加值</b></td>";
...
echo "<span style=\"background-color:  ". $UsrGenrl['color'] ."\"> <b>装备</b> </span> ";
if ($UsrGenrl['msuit'])
echo "$SysWepA[name] 经验: $UsrWepA[1]<br>备用一: $SysWepB[name] 经验: $UsrWepB[1]<br>备用二: $SysWepC[name] 经验: $UsrWepC[1]";
else echo "<br>没有机体。<br><br><br>";
echo "<br><span style=\"background-color:  ". $UsrGenrl['color'] ."\">&nbsp<b>辅助装备</b> </span> ";
if ($UsrGenrl['msuit'])
echo "$SysWepD[name] 经验: $UsrWepD[1]";
if ($PlGameVal['p_equip'] && $PlGameVal['p_equip'] != '0<!>0'){
echo "<br><span style=\"background-color:  ". $UsrGenrl['color'] ."\">&nbsp<b>常规装备</b> </span> ";
if ($UsrGenrl['msuit'])
echo "$SysWepE[name] 经验: $UsrWepE[1]";
}
...
echo "        {status_now.innerText='可发进';status_now.style.color='#143DC1';}";
...
echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "<p><span style=\"font-weight: 700; font-size: 10pt; background-color:  ". $UsrGenrl['color'] ."\"> <b>邀请信</b> </span><br>";
echo "$UsrGenrl[request]";
echo "<input type=submit onClick=\"actionc.value='Accept'\" value='答应'>";
echo "<input type=submit onClick=\"actionc.value='Refuse'\" value='拒绝'>";
echo "</form>";
}

if ($Pl_Org['opttime'] > $CFU_Time && $Pl_Org['optmissioni']){
echo "<p><span style=\"font-weight: 700; font-size: 10pt; background-color:  ". $Pl_Org['color'] ."\"> <b>出击通知书</b> </span><br>";
echo "<font style=\"font-size: 10pt;color: white\">[任务]</font><font style=\"font-size: 8pt;\">行动代号: $Pl_Org[operation]<br>";
...
echo "<font style=\"font-size: 10pt;color: white\">[内容]</font>攻击属于 <font color=$Opt_Org[color]>$Opt_Org[name]</font> 统治下的",$Opt_Area["Sys"]["map_id"],"区域";
$StartAtk = cfu_time_convert($Pl_Org['optstart']);
$TimeEnd = cfu_time_convert($Pl_Org['opttime']);
echo "<br><font style=\"font-size: 10pt;color: white\">[开始时间]</font> $StartAtk <br><font style=\"font-size: 10pt;color: white\">[完结时间]</font> $TimeEnd ";
}
echo "</font>";
}
...
echo "</script>";
echo "<p><span style=\"font-weight: 700; font-size: 10pt; background-color:  ". $UsrGenrl['color'] ."\"> <b>历程纪录</b> <a href=\"Javascript:showlog();\" id=logbtn style=\"text-decoration: none\">[+]</a></span><br><div id=logspc style=\"font-size: 8pt;position: absolute;visibility: hidden\">";
$User_Log=GetUsrLog("$UsrGenrl[username]") or die ('无法取得纪录资讯！！<br>请联络管理员！');
...
echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "<input style=\"$BStyleA\" $BStyleB type=button value='战斗' onClick=\"movebattle()\">";
echo "<input style=\"$BStyleA\" $BStyleB type=button value='移动' onClick=\"act.action='map.php?action=Move';actionb.value='A';act.target='Beta';act.submit();\">";
if ($Area["User"]["hp"] <= 0 && $PlGameVal['rights'] == '1' && ereg_replace('(Atk=\()|\)','',$Pl_Org['optmissioni']) == $UsrGenrl['coordinates'] && $CFU_Time < $Pl_Org['opttime'])
echo "<input style=\"$BStyleA\" $BStyleB type=button value='占领' onClick=\"act.action='organization.php?action=TakeCity';actionb.value='A';act.target='Beta';act.submit();\">";
echo "　<input style=\"$BStyleA\" $BStyleB type=button value='装备' onClick=\"act.action='equip.php?action=equip';act.target='Beta';act.submit();\">";
echo "<input style=\"$BStyleA\" $BStyleB type=button value='机体生产' onClick=\"act.action='equip.php?action=buyms';act.actionb.value='buyms';act.target='Beta';act.submit();\">";
echo "<input style=\"$BStyleA\" $BStyleB type=button value='机体改造' onClick=\"act.action='statsmod.php?action=modms';act.actionb.value='buyms';act.target='Beta';act.submit();\">";
...
echo "<input style=\"$BStyleA\" $BStyleB type=button value='修理工场' onClick=\"act.action='statsmod.php?action=repairms';act.actionb.value='sel';act.target='Beta';act.submit();\">";
echo "　<input style=\"$BStyleA\" $BStyleB type=button value='战术学院' onClick=\"act.action='tacticslearn.php?action=main';act.actionb.value='none';act.target='Beta';act.submit();\">";
echo "　<input style=\"$BStyleA\" $BStyleB type=button value='仓库' onClick=\"act.action='warehouse.php?action=selection';act.actionb.value='none';act.target='Beta';act.submit();\">";
echo "<input style=\"$BStyleA\" $BStyleB type=button value='银行' onClick=\"act.action='bank.php?action=main';act.actionb.value='none';act.target='Beta';act.submit();\">";
echo "<input style=\"$BStyleA\" $BStyleB type=button value='商场' onClick=\"act.action='market.php?action=main';act.actionb.value='none';act.target='Beta';act.submit();\">";
...
echo "<input style=\"$BStyleA\" $BStyleB type=button value='情报' onClick=\"act.action='information.php?action=Main';act.actionb.value='';act.target='Beta';act.submit();\">";
...
echo "<input style=\"$BStyleA\" $BStyleB type=button value='退出' onClick=\"location.replace('index2.php');\">";
echo "</td>";
echo "</tr></form>";
echo "</table>";
