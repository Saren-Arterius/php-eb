...
$t_now = time();
if ($Pl_Settings['btltime'] == $t_now){echo "动作不合理过快，强制停止运作1小时。";postFooter();mysql_query("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_general_info` SET `btltime` = ".intval($t_now+3600)." WHERE `username` = '$GameVal[username]' LIMIT 1;");exit;}
elseif ($t_now - $Pl_Settings['btltime'] <= 1){echo "动作过快。";postFooter();mysql_query("UPDATE `".$GLOBALS['DBPrefix']."phpeb_user_general_info` SET `btltime` = ".intval($t_now+10)." WHERE `username` = '$GameVal[username]' LIMIT 1;");exit;}
...
else {echo "未定义的动作。";postFooter();exit;}
...
echo "<p align=center style=\"font-size: 16pt\">装备完成了！<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"><input type=submit value=\"继续装备\" onClick=\"frmeq.submit()\"></p>";
echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
...
if ($GenVal['cash'] - $BuyWepS['price'] < 0){echo "金钱不足!!<br>";PostFooter();exit;}
if (!ereg('.*B.*',$BuyWepS['kind'])){echo "不能购买此武器。<br>";PostFooter();exit;}
if (ereg('(CannotEquip)+',$syswepbuyinfo['spec']) && !$UsrWepA[0]){echo "不能购买此武器。<br>";PostFooter();exit;}
$GenVal['cash'] = $GenVal['cash'] - $BuyWepS['price'];

if($UsrWepA[0] == '0') {$UsrWepA[0] = $BuyWepS['id'];$UsrWepA[1] = 0;$Pos_Flag='你现在正使用这新的武器。';}
elseif($UsrWepB[0] == '0') {$UsrWepB[0] = $BuyWepS['id'];$UsrWepB[1] = 0;$Pos_Flag='新的武器装了在备用一。';}
elseif($UsrWepC[0] == '0') {$UsrWepC[0] = $BuyWepS['id'];$UsrWepC[1] = 0;$Pos_Flag='新的武器装了在备用二。';}
else $ERROR_Buy_Wep = 'True';

if ($ERROR_Buy_Wep){echo "购买时发生错误，购买中止。<br>";PostFooter();exit;}
...
echo "</form>";
echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
echo "<p align=center style=\"font-size: 16pt\">购买完成了！$Pos_Flag<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"><input type=submit value=\"继续装备\" onClick=\"frmeq.submit()\"></p>";
echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
...
echo "</form>";
echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
echo "<p align=center style=\"font-size: 16pt\">售出完成了！<br>你得到了资金 $SellP 元。<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"><input type=submit value=\"继续装备\" onClick=\"frmeq.submit()\"></p>";
echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
...
if(!is_numeric($price) || $price < 1){echo "价格非法！";PostFooter();exit;}
...
echo "<p align=center style=\"font-size: 16pt\">托售完成了！要等买主从二手市场买走你托售的物品才会进帐。<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"><input type=submit value=\"继续装备\" onClick=\"frmeq.submit()\"></p>";
echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
...
if ($GameVal[$slot_sw] && $GameVal[$slot_sw] != '0<!>0' && $slot_sw != 'eqwep'){
        if ($slot_sw == 'wepa'){echo "请勿赏试把装备中的武器装入辅助装备。";postFooter();exit;}
...
elseif($GameVal[$slot_sw] && $slot_sw == 'eqwep'){
        ...
        else {echo "没有空位卸下装备。";postFooter();exit;}
        unset($SRFlag,$TFlag);
}
else {echo "未定义的动作。";postFooter();exit;}
...
echo "<td width=\"195\">机体名称</td>";
echo "<td width=\"50\">攻击修正</td>";
echo "<td width=\"50\">防御修正</td>";
echo "<td width=\"50\">反应修正</td>";
echo "<td width=\"50\">命中修正</td>";
echo "<td width=\"50\">生命加成</td>";
echo "<td width=\"55\">生命回复</td>";
echo "<td width=\"50\">能量加成</td>";
echo "<td width=\"55\">能量回复</td>";
echo "<td width=\"30\">要求等级</td>";
...
echo "选购机体: <select onChange=\"switchmsImg();location.replace('#imagetop')\" name=BuyMsDesired";
if ($GenVal['msuit']) {echo " disabled = true"; $additionaltext = "<br><font style=\"font-size: 14px;color: red\"><b>你已有机体，请先张其解体再购入。</b>";}
echo ">";
echo "<option value=''>---选择机体---$BuyMsPossibilities</select>";
echo "<script language='Javascript'>";
echo "function returncheckbuyms(){";
echo "if(document.buymsform.BuyMsDesired.selectedIndex == ''){alert('没有选好机体。');return false;}";
echo "if(priceselctd.innerText > $GenVal[cash]){alert('金钱不足!!');return false;}";
echo "else{if (confirm('购买'+msnamesel.innerText+'需要 '+priceselctd.innerText+'元。\\n确定要购买吗？') == true){return true;}else{return false;}}";
echo "}</script>";
echo "<input name=buymsbutton type=submit onClick=\"return returncheckbuyms();\" value='购买'";
if ($GenVal['msuit']) echo " disabled = true";
echo ">";
}else echo "<tr align=center><td colspan=12>没有可购买的机体";
...
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
        ...
        echo "if (confirm('确定要解体吗？') == true){sellmsform.submit()}else{return false}";
        echo "}";
        echo "</script>";
echo "<tr align=center valign=bottom><td><b>机体解体</b></td></tr>";
...
echo "<tr><td>把此机体解体能够获得资金 $SellPrice 元。<br>进行解体: <input type=button value='确定' onclick='cfmsell();'></td></tr>";
echo "</form></table>";
}
postFooter();
}
...
echo "</form>";
echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
echo "<p align=center style=\"font-size: 16pt\">改造完成了！<br><input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"><input type=submit value=\"继续装备\" onClick=\"frmeq.submit()\"></p>";
echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
echo "</form>";
postFooter();
}else {echo "此武器无法改造成目标武器或此使用武器的经验不足！";postFooter();exit;}
...
GetUsrDetails("$Pl_Value[USERNAME]",'GenVal','GameVal');
if ($actionc != 'sell'){
if ($GenVal['msuit']){echo "已经有机体!!<br>";PostFooter();exit;}
GetMsDetails("$BuyMsDesired",'BuyMsDVal');
if ($GenVal['cash'] - $BuyMsDVal['price'] < 0){echo "金钱不足!!<br>";PostFooter();exit;}
if ($GameVal['level'] < $BuyMsDVal['needlv']){echo "等级不足!!<br>";PostFooter();exit;}
...
if (!$GenVal['msuit']){echo "你没有机体!!<br>";PostFooter();exit;}
$SellPrice = Floor($NowMS['price']/2 + $NowMS['price']*0.2);
...
mysql_query($sql);
echo "<form action=gmscrn_main.php?action=proc method=post name=frmreturn target=Alpha>";
echo "<p align=center style=\"font-size: 16pt\">解体完成了！<br>你得到资金 $SellPrice 元。<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
