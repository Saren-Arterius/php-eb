...
unset($torder);
$sql_gen_results = mysql_query ($sql_gen) or die ('出错1, 原因:' . mysql_error() . '<br>');
$numofoppos=mysql_num_rows($sql_gen_results);
        ...
        echo "<tr align=center><td colspan=8><b>对手列表: </b></td></tr>";
        echo "<tr align=center>";
        echo "<td width=\"20\">No.</td>";
        echo "<td width=\"150\">对手名称</td>";
        echo "<td width=\"30\">等级</td>";
        echo "<td width=\"90\">类型</td>";
        echo "<td width=\"100\">机体</td>";
        echo "<td width=\"50\">状态</td>";
        echo "<td width=\"30\">战斗</td>";
        echo "</tr>";
        echo "<script language=\"Javascript\">";
        echo "function cfmAtkOnline(){";
        echo "if (confirm('攻击在线的玩家可能会有损您的名声，真的要这样做？')==true){return true}";
...
for ($counter=1;$counter<=$numofoppos;$counter++){
        ...
        $sql_game_results = mysql_query ($sql_game) or die ('出错2, 原因:' . mysql_error() . '<br>');
        ...
        if ($Area_Org['id'] == $Op_Org['id'] && $AttackFort && $Op_Org['id']){echo "<font color='red'> [敌]</font>";$OrgWarOppos = '1';$war_c++;}
        ...
        echo "<font color='red'> [敌]</font>";$OrgWarOppos = '1';}
        ...
        if ($CFU_Time-$Op_Gen['time2'] < $Offline_Time){
                $OnlineStat = "<b style=\"color: red\">参战中</b>";
        ...
        if ($mini_c == 0){echo "<tr align=center><td colspan=7 align=center>全部玩家也修理中或暂时没有任何对手！</td></tr>";$DisAtkButton=' disabled';}
        if ($war_c == 0 && $AttackFort == 'True' && $Area["User"]["hp"] > 0){echo "<tr align=center><td colspan=6 align=center>可以攻击要塞！</td>";
        ...
        echo "开启EXAM System<input type=checkbox name=EXAMStat";
        ...
        if (($Pl_Gen['hypermode'] == 1 || $Pl_Gen['hypermode'] == 5) && $ControlSEED) $TactInf['spc'] = ceil($TactInf['spc'] * 1.25) + 5; //SEED Mode SP额外消耗
        if ($Pl_Gen['hypermode'] >= 4 && $Pl_Gen['hypermode'] <= 6) $TactInf['spc'] = ceil($TactInf['spc'] * 1.20) + 3; //EXAM System SP额外消耗
        ...
        echo "</select>";
        echo "<a name=StartBattle>攻击目标:</a> ";
        echo "<input type=submit $BStyleB style=\"$BStyleA\" name=battle_submit value='确定' OnClick=\"this.style.visibility='hidden';\"";
        echo $DisAtkButton;
        echo ">   ";
        echo "";
        echo "</td>";
