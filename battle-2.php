...
{        unset($AtkFortFlag);
        if (!$Op_Name){echo "请先挑选对手！";exit;}
        elseif ($Op_Name == $Pl_Value['USERNAME']){echo "不能攻击自己！";exit;}
        ...
        {echo "不能攻击要塞";postFooter();exit;}$AtkFortFlag='1';
        ...
        $Op_Game = Array("username" => '<AttackFort>',"gamename" => '防御要塞',"hp" => $Area["User"]["hp"],
        ...
        if ($Op_Game['status']){echo "对手修理中．．";postFooter();exit;}
        if ($Pl_Game['status']){echo "修理中，无法出击。";postFooter();exit;}

        if ($Op_Gen['coordinates'] != $Pl_Gen['coordinates']){echo "看不见对手．．<br>对方位置: $Op_Gen[coordinates] 自己位置: $Pl_Gen[coordinates]";postFooter();exit;}
        ...
        if (($Pl_Gen['hypermode'] == 1 || $Pl_Gen['hypermode'] == 5) && $ControlSEED) $SP_Cost = ceil($SP_Cost * 1.25) + 5;        //SEED Mode SP额外消耗
        if ($Pl_Gen['hypermode'] >= 4 && $Pl_Gen['hypermode'] <= 6) $SP_Cost = ceil($SP_Cost * 1.20) + 3;                        //EXAM System SP额外消耗
        $SP_CostOP = 0;
        ...
        $Op_Ms = Array(        "msname" => "$Pl_Gen[coordinates]区域要塞","atf" => "0","def" => "0","ref" => "0","taf" => "0",
        ...
        if (!$Pl_WepA[0]) {echo "你没有装备武器，不能出击。";postFooter();exit;}
        elseif ($Pl_Game['en'] < ($Pl_SyWepA['enc']+$Pl_SyWepD['enc'])) {echo "EN不足，无法出击。";postFooter();exit;}
        elseif ($Pl_Game['sp'] < $SP_Cost) {echo "SP不足，无法以 $Pl_Tactics[name] 出击。";postFooter();exit;}
        elseif (!$Pl_Tactics['name']) {echo "不明的战术。";postFooter();exit;}
        //Specs - Type Customs
        ...
        echo "</tr>";
        echo "<tr valign=center align=center>";
        echo "<td><br>机体:$Pl_Ms[msname]<br></td>";
        echo "<td><br>机体:$Op_Ms[msname]<br></td>";
        echo "</tr>";
        //Include Functions
        ...
        //Get Hit and Miss Values
                ...
                //Specs
                        ...
                        //Type Lower-Case Specs of Miss Values
                                ...
                                if(ereg('(AntiMobS)+',$Pl_Spec) && mt_rand(0,100) >= 50) {$Pl_MobSGd=6; $Spec_Event_Tag .= "<Br>你成功干扰对方的行动！";}
                                if(ereg('(AntiMobS)+',$Op_Spec) && mt_rand(0,100) >= 50) {$Op_MobSGd=6; $Spec_Event_Tag .= "<Br>对方成功干扰你的行动！";}
                        ...
                        //Type Lower-Case Specs of Hit Values
                                ...
                                if(ereg('(AntiTarS)+',$Pl_Spec) && mt_rand(0,100) >= 50) {$Pl_TarSGd=6; $Spec_Event_Tag .= "<Br>你成功干扰对方的雷达！";}
                                if(ereg('(AntiTarS)+',$Op_Spec) && mt_rand(0,100) >= 50) {$Op_TarSGd=6; $Spec_Event_Tag .= "<Br>对方成功干扰你的雷达！";}
        ...
        //Get Defence Values
                ...
                if(ereg('(DefX)+',$Pl_Spec) || $Pl_Gen['hypermode'] == 3){if($Pl_DefX_pc >= 40) {$Pl_Def += 20; $Spec_Event_Tag .= "<Br>你的底力爆发了！";}}
                if(ereg('(DefX)+',$Pl_Spec) && $Pl_Gen['hypermode'] == 3){if($Pl_DefX2_pc >= 40) {$Pl_Def += 20; $Spec_Event_Tag .= "<Br>你的念动力之底力爆发了！";}}
                if(ereg('(DefX)+',$Op_Spec) || $Op_Gen['hypermode'] == 3){if($Op_DefX_pc >= 40) {$Op_Def += 20; $Spec_Event_Tag .= "<Br>对手的底力爆发了！";}}
                if(ereg('(DefX)+',$Op_Spec) && $Op_Gen['hypermode'] == 3){if($Op_DefX2_pc >= 40) {$Op_Def += 20; $Spec_Event_Tag .= "<Br>对手的念动力之底力爆发了！";}}
        ...
        //Get Attacking Values
                ...
                if(ereg('(AtkA)+',$Pl_Spec) && $Spec_AtkA_R >= 50){$Pl_Attacking += 20;$Spec_Event_Tag .= "<Br>你进入了兴奋状态！";}
                if(ereg('(AtkA)+',$Op_Spec) && $Spec_AtkA_R2 >= 50){$Op_Attacking += 20;$Spec_Event_Tag .= "<Br>对手进入了兴奋状态！";}
                        //SEED Mode
        ...
        //Calculate Left HP and EN
                ...
                //Spec - First Strike
                        if(ereg('(FirstStrike)+',$Pl_Spec) && !ereg('(FirstStrike)+',$Op_Spec) && mt_rand(0,100) <= (85+($Pl_Game['level']-$Op_Game['level'])) && floor($Op_Game['hp'] - $Pl_Dealt) <= 0){
                                $Op_Dealt=0;
                                $CStrikeRds=0;
                                $Spec_Event_Tag .="<br>你的先制攻击把对方即时击败！";
                        }
                        if(ereg('(FirstStrike)+',$Op_Spec) && !ereg('(FirstStrike)+',$Pl_Spec) && mt_rand(0,100) <= (85+($Op_Game['level']-$Pl_Game['level'])) && floor($Pl_Game['hp'] - $Op_Dealt) <= 0){
                                $Pl_Dealt=0;
                                $StrikeRds=0;
                                $Spec_Event_Tag .="<br>对方的先制攻击把你即时击败！";
                        }
                //Normal
                ...
                //Special Event Status
                        ...
                        if(ereg('[DamA]{4}',$Pl_SyWepA['spec']) && $S_Event_ENdam_Rand >= 85 && $VictoryFlag!=2){$Op_Resulting_EN=floor($Op_Resulting_EN*0.5);$Spec_Event_Tag .="<br>对手的机体被损坏，剩餘能源下降！";}
                        elseif(ereg('[DamA]{4}',$Pl_SyWepA['spec']) && $S_Event_ENdam_Rand >= 95 && $VictoryFlag==2){$Op_Resulting_EN=floor($Op_Resulting_EN*0.5);$Spec_Event_Tag .="<br>对手的机体被损坏，剩餘能源下降！";}
                        if(ereg('[DamA]{4}',$Op_SyWepA['spec']) && $S_Event_ENdam_Rand2 >= 85 && $VictoryFlag!=1){$Pl_Resulting_EN=floor($Pl_Resulting_EN*0.5);$Spec_Event_Tag .="<br>你的机体被对手损坏，剩餘能源下降！";}
                        elseif(ereg('[DamA]{4}',$Op_SyWepA['spec']) && $S_Event_ENdam_Rand2 >= 90 && $VictoryFlag==1){$Pl_Resulting_EN=floor($Pl_Resulting_EN*0.5);$Spec_Event_Tag .="<br>你的机体被对手损坏，剩餘能源下降！";}
        //Weapon Stats
                //Nil
        ...
        //Analyze Results
                ...
                $sql = ("INSERT INTO `".$GLOBALS['DBPrefix']."phpeb_game_history` (`time`, `history`) VALUES (UNIX_TIMESTAMP(), '<font color=$Pl_Org[color]>$Pl_Org[name]</font>的<font color=$Pl_Gen[color]>$Pl_Game[gamename]</font>，成功\\攻陷由<font color=$Area_Org[color]>$Area_Org[name]统治下的$Pl_Gen[coordinates]区域</font>！');");
        ...
        //Update Player Status
                switch ($VictoryFlag){case '1': $Op_Game['status']='1';$Pl_Game['victory']+=1;$Pl_Game['v_points']+=1;$Result_Tag .="<br>你把对手击败！";$Pl_Log_Tag="对手的$Op_Ms[msname]被你击败！";$Op_Log_Tag="你被对手的$Pl_Ms[msname]击败。";break;
                case '2': $Pl_Game['status']='1';$Result_Tag .="<br>你被对手击败。";$Pl_Log_Tag="你被对手的$Op_Ms[msname]击败。";$Op_Log_Tag="对手的$Pl_Ms[msname]被你击败！";break;
                case '3': $Op_Game['status']='1';$Pl_Game['status']='1';$Result_Tag .="<br>你与对手两败俱伤。";$Pl_Log_Tag="你与对手两败俱伤。";$Op_Log_Tag="对手与你两败俱伤。";break;
                default: $Result_Tag .="<br>你与对手的战斗没有分出胜负。";$Pl_Log_Tag="你与对手的战斗没有分出胜负。";$Op_Log_Tag="对手与你的战斗没有分出胜负。";};
                //Special Event Status
                        //武器特效 - 战斗不能
                        ...
                        if(ereg('(DamB)+',$Pl_SyWepA['spec']) && $S_Event_Status_Rand >= 90 && $VictoryFlag==0){$Op_Game['status']='1';$Spec_Event_Tag .="<br>对手的机体被损坏，战斗不能！";}
                        elseif(ereg('(DamB)+',$Pl_SyWepA['spec']) && $S_Event_Status_Rand >= 98 && $VictoryFlag==2){$Op_Game['status']='1';$Spec_Event_Tag .="<br>对手的机体被损坏，战斗不能！";}
                        }
                        if(ereg('(AntiDam)+',$Pl_Spec))$S_Event_Status_Rand2=0;
                        else {
                        if(ereg('(DamB)+',$Op_SyWepA['spec']) && $S_Event_Status_Rand2 >= 90 && $VictoryFlag==0){$Pl_Game['status']='1';$Spec_Event_Tag .="<br>你的机体被对手损坏，战斗不能，需要修理！";}
                        elseif(ereg('(DamB)+',$Op_SyWepA['spec']) && $S_Event_Status_Rand2 >= 95 && $VictoryFlag==1){$Pl_Game['status']='1';$Spec_Event_Tag .="<br>你的机体被对手损坏，战斗不能，需要修理！";}
                        }
        //Level Up
                ...
                if ($Pl_Game['expr'] >= $UserNextLvExp){$Pl_Game['level'] += 1;$Pl_Game['spmax'] += 1;if($Pl_Game['level']%10==0)$Pl_Game['spmax'] += 5;$Pl_Game['expr'] = 0;$Pl_Gen['growth'] = $Pl_Gen['growth'] + $Pl_Stat_Gain;$Result_Tag .="<br>你升了级！<br>获得 $Pl_Stat_Gain 点成长点数！";$UpD_Pl_level='1';}
                if ($Op_Game['expr'] >= $OppoNextLvExp){$Op_Game['level'] += 1;$Op_Game['spmax'] += 1;if($Op_Game['level']%10==0)$Op_Game['spmax'] += 5;$Op_Game['expr'] = 0;$Op_Gen['growth'] = $Op_Gen['growth'] + $Op_Stat_Gain;$Result_Tag .="<br>对手升了级！";$UpD_Op_level='1';}
        //Type Level Up
                $TypeRank=0;
                ...
                $TypeCH_Q = mysql_query($sqltypeup) or die('无法更新基本资讯1, 原因:' . mysql_error() . '<br>');
                $TypeCH_New = mysql_fetch_array($TypeCH_Q) or die('无法更新基本资讯2, 原因:' . mysql_error() . '<br>');
        ...
        //Update Information
                ...
                mysql_query($sqlgen) or die ('无法取得基本资讯, 原因:' . mysql_error() . '<br>' . postFooter());
                mysql_query($sqlgenop) or die ('无法取得基本资讯, 原因:' . mysql_error() . '<br>' . postFooter());
                ...
                mysql_query($sqlgame) or die ('无法更新游戏资讯( 1 ), 原因:' . mysql_error() . '<br>' . postFooter());
                if ($AtkFortFlag!='1')
                mysql_query($sqlgameop) or die ('无法更新游戏资讯( 2 ), 原因:' . mysql_error() . '<br>' . postFooter());
                ...
                $sqllog .= ("`log1` = '你与$Op_Game[gamename]交战！$Pl_Log_Tag',".$TmpLogVar[3].$TmpLogVar[2].$TmpLogVar[1].$TmpLogVar[0]);
                ...
                $sqllogop .= ("`log1` = '$Pl_Game[gamename]与你交战！$Op_Log_Tag',".$TmpLogVar[3].$TmpLogVar[2].$TmpLogVar[1].$TmpLogVar[0]);
                $sqllogop .= ("`time1` = '$t_now' WHERE `username` = '$Op_Gen[username]' LIMIT 1;");
                mysql_query($sqllog) or die ('无法取得基本资讯, 原因:' . mysql_error() . '<br>' . postFooter());
                if ($AtkFortFlag!='1')
                mysql_query($sqllogop) or die ('无法取得基本资讯, 原因:' . mysql_error() . '<br>' . postFooter());
                }}
                //End of Write Logs

        //Echo Results
        echo "<tr align=center>";
        echo "<td>武器: $Pl_SyWepA[name] 经验: $Pl_WepA[1]<br>";
        if ($Pl_WepD[0]) echo "辅助装备: $Pl_SyWepD[name] 经验: $Pl_WepD[1]<br>";
        if ($Pl_WepE[0]) echo "常规装备: $Pl_SyWepE[name] 经验: $Pl_WepE[1]<br>";
        echo "</td><td>武器: $Op_SyWepA[name] 经验: $Op_WepA[1]<br>";
        if ($Op_WepD[0]) echo "辅助装备: $Op_SyWepD[name] 经验: $Op_WepD[1]<br>";
        if ($Op_WepE[0]) echo "常规装备: $Op_SyWepE[name] 经验: $Op_WepE[1]<br>";
        echo "</td></tr>";
        echo "<tr align=center>";
        echo "<td width=50%>";
        ...
        elseif($OpNoENFlag){echo "能源不足！！";}
        else echo "--- 反射 ---";
        echo"</td>";
        echo "</tr>";
        echo "<tr align=center>";
        echo "<td>$Pl_Tactics[name]<br>";
        if($StrikeRds) echo "你击中对手 $StrikeRds 次，并造成 $Pl_Dealt 点伤害。</td>";
        elseif(ereg('[MirrorDam]{9}',$Pl_SyWepA['spec']) && $Pl_Dealt)  echo "反射伤害，造成 $Pl_Dealt 点伤害。</td>";
        else echo "你未能击中对手！</td>";
        echo "<td>$Op_Tactics[name]<br>";
        if($CStrikeRds && !$OpNoENFlag) echo "对手击中你 $CStrikeRds 次，并造成 $Op_Dealt 点伤害。</td>";
        elseif($OpNoENFlag == '1'){echo "反击不能。</td>";}
        elseif(ereg('[MirrorDam]{9}',$Op_SyWepA['spec']) && $Op_Dealt)  echo "反射伤害，造成 $Op_Dealt 点伤害。</td>";
        else echo "对手未能击中你！</td>";
        ...
        echo "<br>生命值: <span id=Pl_Res_Hp>$Pl_Game[hp]</span>/$Pl_Game[hpmax]<br>消耗能量: ".number_format($Pl_SyWepA['enc']+$Pl_SyWepD['enc']+$Pl_SyWepE['enc'])."</td>";
        echo "<td>";
        ...
        echo "得到 $Pl_Gain_Exp 点经验值和武器经验 $Pl_Gain_Wep_Exp 点。<br>";
        echo "获得战利金 $Pl_Gain_Money 元。";
        if ($Gain_BountyFlag)
        echo "另外获得 $Gain_Bounty 元的悬赏金。";
        echo "$Result_Tag";
        echo "$Spec_Event_Tag";
        echo "</td>";
        echo "</tr>";
        ...
        echo "<input type=submit value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php')\"></p>";
