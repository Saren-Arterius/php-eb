<?php
        ...
        echo "<td width=\"85\">原购入价</td>";
        echo "<td width=\"85\">改造武器</td>";
        echo "</tr>";
        echo "<script language=\"Javascript\">";
        echo "function cfmev(slot,aevto){";
        echo "if (confirm('确定要改造此武器吗？') == true){evwepform.evfrom.value=slot;evwepform.evto.value=aevto;evwepform.submit();}else{return false}";
        echo "}";
        echo "</script>";
        ...
        echo "<td width=\"85\"><input type=button value=确认改造 onClick=\"cfmev('wepa','$NextEv_A[$c_ev]');\"";
        ...
        echo "<td width=\"85\"><input type=button value=确认改造 onClick=\"cfmev('wepb','$NextEv_B[$c_ev]');\"";
        ...
        echo "<td width=\"85\"><input type=button value=确认改造 onClick=\"cfmev('wepc','$NextEv_C[$c_ev]');\"";
        if(ereg('(CannotEquip)+',$EvWepE_C['spec']) || ereg('(FortressOnly)+',$EvWepE_C['spec'])) echo " disabled";
        echo "></td></tr>";$AvailEvFlag='1';}                }
        if (!$AvailEvFlag){echo "<tr align=center><td colspan=8>暂时没有任何有改造可能性的武器。</td></tr>";}
        echo "</form></table>";
        echo "<br><hr width=75%>";        }
        ...
        echo "if (confirm('确定要特殊强化此武器吗？') == true){sevwepform.evfrom.value=slot;sevwepform.evto.value=aevto;sevwepform.submit();}else{return false}";
        echo "}";
        echo "</script>";
        echo "<br><table align=center border=\"1\" cellpadding=\"0\" cellspacing=\"0\" style=\"border-collapse: collapse\" bordercolor=\"#FFFFFF\" width=\"665\">";
        ...
        echo "<input type=hidden value='$Pl_Value[USERNAME]' name=Pl_Value[USERNAME]>";
        echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
        echo "<tr align=center><td colspan=8><b>武器特殊强化: </b></td></tr>";
        echo "<tr align=center>";
        echo "<td width=\"195\">武器名称</td>";
        echo "<td width=\"80\">攻击力</td>";
        echo "<td width=\"30\">命中</td>";
        ...
        echo "<tr align=center><td colspan=8><b><font color='yellow'>$SysWepE_A[name]</font> 特殊强化的可能性: </b></td></tr>";
        for($c_ev_a=1;$c_ev_a<= count($NextEv_A);$c_ev_a++){$c_ev=$c_ev_a-1;
        GetWeaponDetails("$NextEv_A[$c_ev]","EvWepE_A");
        echo "<tr align=center>";
        echo "<td width=\"195\">$EvWepE_A[name]</td>";
        ...
        echo "<td width=\"120\">$EvWepASpecs</td>";
        echo "<td width=\"85\">$EvWepE_A[price]</td>";
        echo "<td width=\"85\"><input type=button value=特殊强化 onClick=\"cfmsev('wepa','$NextEv_A[$c_ev]');\"";
        if(ereg('(CannotEquip)+',$EvWepE_A['spec']) || ereg('(FortressOnly)+',$EvWepE_A['spec']) || ereg('(RawMaterials)+',$EvWepE_A['spec'])) echo " disabled";
        echo "></td></tr>";$AvailSevFlag = '1';}                }
        if ($NextEv_B[0] != 0 && $UsrWepB[1] >= 2000)        {
        echo "<tr align=center><td colspan=8><b><font color='yellow'>$SysWepE_B[name]</font> 特殊强化的可能性: </b></td></tr>";
        for($c_ev_b=1;$c_ev_b<= count($NextEv_B);$c_ev_b++){$c_ev=$c_ev_b-1;
        GetWeaponDetails("$NextEv_B[$c_ev]","EvWepE_B");
        echo "<tr align=center>";
        echo "<td width=\"195\">$EvWepE_B[name]</td>";
        ...
        echo "<td width=\"120\">$EvWepBSpecs</td>";
        echo "<td width=\"85\">$EvWepE_B[price]</td>";
        echo "<td width=\"85\"><input type=button value=特殊强化 onClick=\"cfmsev('wepb','$NextEv_B[$c_ev]');\"";
        if(ereg('(CannotEquip)+',$EvWepE_B['spec']) || ereg('(FortressOnly)+',$EvWepE_B['spec'])) echo " disabled";
        echo "></td></tr>";$AvailSevFlag = '1';}                }
        if ($NextEv_C[0] != 0 && $UsrWepC[1] >= 2000)        {
        echo "<tr align=center><td colspan=8><b><font color='yellow'>$SysWepE_C[name]</font> 特殊强化的可能性: </b></td></tr>";
        for($c_ev_c=1;$c_ev_c<= count($NextEv_C);$c_ev_c++){$c_ev=$c_ev_c-1;
        GetWeaponDetails("$NextEv_C[$c_ev]","EvWepE_C");
        echo "<tr align=center>";
        echo "<td width=\"195\">$EvWepE_C[name]</td>";
        ...
        $EvWepCSpecs = ReturnSpecs($EvWepE_C['spec']);
        echo "<td width=\"85\">$EvWepE_C[price]</td>";
        echo "<td width=\"85\"><input type=button value=特殊强化 onClick=\"cfmsev('wepc','$NextEv_C[$c_ev]');\"";
        if(ereg('(CannotEquip)+',$EvWepE_C['spec']) || ereg('(FortressOnly)+',$EvWepE_C['spec'])) echo " disabled";
        echo "></td></tr>";$AvailSevFlag = '1';}                }
        if (!$AvailSevFlag){echo "<tr align=center><td colspan=8>暂时没有任何有特殊强化可能性的武器。</td></tr>";}
        echo "</form></table>";
        echo "<br><hr width=75%>";        }
        //End Spec Ev
        if ($GenVal['msuit'] != 0){
        ...
        echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
        echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
        echo "<tr align=center><td colspan=8><b>装备武器: </b></td></tr>";
        echo "<tr align=center>";
        echo "<td width=\"195\">武器名称</td>";
        echo "<td width=\"80\">攻击力</td>";
        echo "<td width=\"30\">命中</td>";
        ...
        echo "<td width=\"85\"><input type=submit value=装备此武器 onClick=\"equipwepform.slot_sw.value='wepb';\"";
        ...
        echo "<td width=\"85\"><input type=submit value=装备此武器 onClick=\"equipwepform.slot_sw.value='wepc';\"";
        ...
        echo "<td width=\"85\"><input type=submit value=位置转换 onClick=\"equipwepform.slot_sw.value='eqwep';\"></td>";
        echo "</tr>";$AvailEqWFlag = '1';}}
        if (!$AvailEqWFlag){echo "<tr align=center><td colspan=8>暂时没有任何以装备的武器。</td></tr>";}
        echo "</form></table>";
        ...
        echo "<input type=hidden value='$Pl_Value[PASSWORD]' name=Pl_Value[PASSWORD]>";
        echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
        echo "<tr align=center><td colspan=8><b>辅助装备: </b></td></tr>";
        echo "<tr align=center>";
        echo "<td width=\"195\">武器名称</td>";
        echo "<td width=\"80\">攻击力</td>";
        echo "<td width=\"30\">命中</td>";
        ...
        $SysWepDSpecs = ReturnSpecs($SysWepD['spec']);
        echo "<td width=\"120\">$SysWepDSpecs</td>";
        echo "<td width=\"85\"><input type=submit value=卸下此装备 onClick=\"equipdefform.slot_sw.value='eqwep';\"></td>";
        echo "</tr>";}
        ...
        $SysWepBSpecs = ReturnSpecs($SysWepB['spec']);
        echo "<td width=\"120\">$SysWepBSpecs</td>";
        echo "<td width=\"85\"><input type=submit value=装备 onClick=\"equipdefform.slot_sw.value='wepb';\"></td>";
        ...
        $SysWepCSpecs = ReturnSpecs($SysWepC['spec']);
        echo "<td width=\"120\">$SysWepCSpecs</td>";
        echo "<td width=\"85\"><input type=submit value=装备 onClick=\"equipdefform.slot_sw.value='wepc';\"></td>";
        echo "</tr>";$AvailEqEFlag = '1';}}
        if (!$AvailEqEFlag){echo "<tr align=center><td colspan=8>暂时没有任何以成为辅助装备的武器。</td></tr>";}
        echo "</form></table>";
        echo "<br><hr width=75%><br>";
...
$wepbuyoptions='';
        ...
        echo "<tr align=center><td colspan=8><b>选购新武器: </b>";
        if ($UsrWepA[0]&&$UsrWepB[0]&&$UsrWepC[0]){
        $disBW = 'disabled';$disBW_mes = '<br>你没有多出来的空间可以放置新武器。';}
        else $disBW = 'enabled';
        echo "<script language=\"Javascript\">";
        echo "function cfmbuy(){";
        echo "if (confirm('确定要购买吗？') == true){buywepform.BuyWepDesired.disabled=false;buywepform.submit()}else{buywepform.BuyWepDesired.disabled=false;buywepform.cfmbuybutton.disabled=false;return false}";
        echo "}";
        echo "</script>";
        echo "<select name=BuyWepDesired $disBW>";
        echo $wepbuyoptions ;
        echo "</select>";
        echo "<input type=button value='购买' name=cfmbuybutton OnClick=\"buywepform.BuyWepDesired.disabled=true;this.disabled=true;cfmbuy()\" $disBW>$disBW_mes";
        //echo "<input type=submit value='购买' name=cfmbuybutton $disBW>$disBW_mes";
        echo "</td></tr>";
        echo "</form></table>";
}
}else echo "没有机体，不能购入新武器！";
...
if (($UsrWepA[0])||($UsrWepB[0])||($UsrWepC[0] || !$UsrWepC[0]))
        ...
        echo "</tr>"; $SellW_Options .= "<option value=WepB>$SysWepB[name](备用武器一)\n";}
        if ($UsrWepC[0]){        
        GetWeaponDetails("$UsrWepC[0]",'SysWepC');
        if ($UsrWepC[2]){
        if ($UsrWepC[2]==1) $SysWepC['name'] = $UsrWepC[3].$SysWepC['name']."<sub>?</sub>";
        ...
        echo "</tr>"; $SellW_Options .= "<option value=WepC>$SysWepC[name](备用武器二)\n";}
        ...
        echo "if (confirm('确定要售出吗？') == true){sellwepform.SellWepDesired.disabled=false;sellwepform.submit()}else{sellwepform.SellWepDesired.disabled=false;sellwepform.cfmsellbutton.disabled=false;return false}";
        echo "}";
        echo "</script>";
        echo "<input type=button value='确定' OnClick=\"sellwepform.SellWepDesired.disabled=true;this.disabled=true;cfmsell()\" name=cfmsellbutton>";
        }else echo "没有可以售出的武器。";
        ...
        echo $SellW_Options;
        echo "</select>";
        echo "价格：<input type=text name='price' size=10 >";
        echo "<script language=\"Javascript\">";
        echo "function cfmsell2(){";
        echo "if (confirm('确定要以这个价格放到二手市场吗？') == true){sellwepform2.SellWepDesired2.disabled=false;sellwepform2.submit()}else{sellwepform2.SellWepDesired2.disabled=false;sellwepform2.cfmsellbutton2.disabled=false;return false}";
        echo "}";
        echo "</script>";
        echo "<input type=button value='确定' OnClick=\"sellwepform2.SellWepDesired2.disabled=true;this.disabled=true;cfmsell2()\" name=cfmsellbutton2>";
        }else echo "没有可以放到二手市场的武器。";
