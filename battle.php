...
if (($CFU_Time - $Pl_Gen['btltime']) < $Btl_Intv){echo "距离上次攻击或移动的时间太短了！<br>请在 ".($Btl_Intv-($CFU_Time - $Pl_Gen['btltime']))." 秒后再进行攻击！";exit;}
...
$WarMessage = "<font color=red>[攻略目标]</font> ";
if ($CFU_Time > $Pl_Org['optstart'])
$AttackFort = 'True';
}

if ($Area["User"]["hp"] <= 0)$FortDestoryedMsg = "<br><font color=red><b>要塞已经沦陷！</b></font>";

if ($mode == 'battle_sel')
{
echo "<hr width=80% style=\"filter:alpha(opacity=100,finishopacity=40,style=2)\">";
echo "<div align=center style=\"font-size: 11pt; color: $Area_Org[color];\">$WarMessage$Area_Org[name]的领地: ". $Area["Sys"]["map_id"] ."区域";
echo "<br><a href=\"Javascript:showfort();\" id=fbtn style=\"text-decoration: none\">要塞状态</a></div>";
...
echo "<br>攻击力: $Area_At 防卫力: $Area_De 命中: $Area_Ta$FortDestoryedMsg";
...
else {echo "<center>你没有机体，不能出击。";postFooter();exit;}
if ($Pl_Game['status']){echo "<center>修理中，无法出击。";postFooter();exit;}
if (!$Pl_WepA[0]) {echo "<center>你没有装备武器，不能出击。";postFooter();exit;}
elseif ($Pl_Game['en'] < ($Pl_SyWepA['enc']+$Pl_SyWepD['enc']+$Pl_SyWepE['enc'])) {echo "EN不足，无法出击。";postFooter();exit;}
switch(mt_rand(1,2)){
